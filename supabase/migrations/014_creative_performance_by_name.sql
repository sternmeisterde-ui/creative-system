-- ============================================================
-- Migration 014: фикс переучёта лидов в creative_performance
--
-- БАГ (до этой миграции): meta_agg группировался по ad_id, а лиды PBI (pbi_agg)
-- агрегировались по ad_name и цеплялись left join'ом по имени. Когда одно имя
-- объявления используется на НЕСКОЛЬКИХ ad_id (в COM ~119 из 237 имён), каждому
-- ad_id-ряду приписывались ВСЕ лиды имени → многократный переучёт, мусорный CPL
-- (€0.003), ложные виннеры, blended CPL схлопывался в ~€1.
--
-- ПРИЧИНА: PBI/Elly данные принципиально привязаны к ИМЕНИ, а не к Meta ad_id
-- (pbi/upload пишет ad_id='pbi:'+name, elly-sync — 'elly:'+ellyId). Реальный Meta
-- ad_id в PBI отсутствует, поэтому джойн по ad_id невозможен.
--
-- ФИКС: гранулярность view = ИМЯ креатива (trim(lower(ad_name))). Meta-метрики
-- суммируются по всем ad_id одного имени, лиды цепляются ОДИН раз. ad_id / campaign
-- / adset / flow — представительные (по максимальному дневному спенду). Порог
-- min_impressions применяется к креативу целиком. Все выходные колонки сохранены 1:1.
-- ============================================================
...
-- Агрегация Meta сразу по ИМЕНИ: суммируем по всем ad_id одного имени.
-- Представительные ad_id/campaign/adset/flow — из строки с макс. дневным спендом.
meta_agg as (
  select
