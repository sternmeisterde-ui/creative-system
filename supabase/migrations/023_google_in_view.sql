-- ============================================================
-- Migration 023: учитывать конверсии Google Ads во view creative_performance
--
-- Google-синк (/api/google/sync) пишет конверсии в pbi_metrics с неймспейсом
-- ad_id = 'google:<campaignId>' (по аналогии с 'elly:%' у Plurio). Но pbi_agg
-- во view фильтровал ТОЛЬКО 'elly:%', поэтому лиды/выручка Google не подхватывались
-- (спенд шёл из meta_ads, а CPL/ROAS оставались пустыми).
--
-- ФИКС: расширяем фильтр pbi_agg на 'google:%'. Остальное идентично миграции 019.
-- Join по-прежнему по trim(lower(ad_name)); неймспейсы elly/google не конфликтуют
-- (elly-sync чистит только 'elly:%', google-sync — свой 'google:%').
-- ============================================================
