-- ============================================================
-- Migration 015: фикс переучёта лидов из Elly (снапшоты складывались)
--
-- БАГ: elly-sync запрашивает у Plurio СУММУ лидов за период (скользящее ~30-дн окно)
-- и штампует ВСЕ строки одной датой dateTo. При регулярном синке на каждый запуск
-- пишется новая строка (ad_id, дата_синка) с тем же ~30-дн итогом. View суммировал
-- leads по имени ПО ВСЕМ датам → складывал N снапшотов одного и того же итога.
-- Пример: 8.04_quiz_CS2_№5 → 4331 «лид» вместо реальных ~468 (×9 переучёт).
--
-- ФИКС (на уровне view, без зависимости от elly-sync): берём для каждого ad_id
-- ТОЛЬКО последний снапшот (по max(date)), затем суммируем по имени. Это убирает
-- дублирование снапшотов во времени. Семантика leads = «последнее окно Elly на ad_id».
-- (Полностью корректный lifetime-учёт потребует дневной разбивки в elly-sync — отдельно.)
--
-- Меняется ТОЛЬКО pbi-часть; остальное идентично миграции 014 (грань по имени).
-- ============================================================
...
-- Последний снапшот лидов на каждый ad_id (а не сумма всех дат-снапшотов).
pbi_latest as (
