-- ============================================================
-- Migration 009: fake_winner detection with risk signals
--
-- Цель: ловить объявления, прошедшие KPI (CPL/CPQL/impressions),
-- но имеющие проблемы внутри воронки или экономики:
--   • low_qual_cr      — Lead→Qual CR < 71% при ≥5 лидах
--   • short_lifespan   — жил <7 дней и сейчас выключен
--   • low_roas_30d     — возраст ≥30 дней, ROAS<1, spend ≥€500
--
-- Категории auto_status: winner | fake_winner | loser | testing
-- ============================================================

-- 1. transactions: счётчик оплат на уровне ad×date.
--    Парсер Elly будет заполнять это поле новой логикой.
alter table pbi_metrics
  add column if not exists transactions integer default 0;

-- 2. Расширить enum winner_status новым значением.
--    В Postgres 12+ ADD VALUE можно вне транзакции одним statement'ом.
alter type winner_status add value if not exists 'fake_winner';

-- 3. Расширить enum alert_type. early_stop уже используется в коде
--    (app/api/analytics/early-stop), но в enum его нет — добавляем.
alter type alert_type add value if not exists 'early_stop';
alter type alert_type add value if not exists 'dead_zero_leads';
alter type alert_type add value if not exists 'fake_winner_detected';

-- 4. Пересоздать view creative_performance с risk_signals + fake_winner.
--    DROP + CREATE вместо CREATE OR REPLACE — порядок колонок меняется
--    (transactions вставляется между revenue и cpl), а REPLACE такое не умеет.
