...
function ConceptForm({ initial, onSave, onCancel }: {
  ...
  onSave: (data: Omit<Concept, "id" | "createdAt">) => void;
  onCancel: () => void;
}) {
...
// AI: generate hypothesis from approved concepts
...
// Known AI-analysis fields from the competitor sheet
...
function buildAdText(row: Record<string, string>): string {
  // If this is the known schema (has ad_archive_id), build rich text from all AI fields
  const parts: string[] = [];
...
function SheetsImportModal({ onClose, onImported }: { onClose: () => void; onImported: () => void }) {
  ...
  const buildAds = () => {
    ...
    return validRows.map(r => {
      const competitor_name = knownSchema ? r.page_name : (r[competitorCol] ?? "");
      const ad_text = knownSchema ? buildAdText(r) : (r[textCol] ?? "");
      const ad_url = r.ad_archive_id
        ? `https://www.facebook.com/ads/library/?id=${r.ad_archive_id}`
        : (r.link_url ?? r.ad_url ?? "");
      return { competitor_name, ad_text, ad_url: ad_url || undefined };
    });
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {error && (
        <div style={{ fontSize: 12, color: "#D96B6B", background: "rgba(217,107,107,0.08)", border: "1px solid rgba(217,107,107,0.2)", borderRadius: 8, padding: "10px 14px" }}>
          {error}
        </div>
      )}
        ...
        <>
          {knownSchema ? (
            <div style={{ fontSize: 12, background: "rgba(110,200,160,0.06)", border: "1px solid rgba(110,200,160,0.2)", borderRadius: 8, padding: "10px 14px", color: "#6EC8A0" }}>
              ✓ Распознана схема с AI-анализом. Загружено {rows.length} строк.<br />
              <span style={{ color: "#666", fontSize: 11 }}>
                Конкурент ← page_name · Текст ← title + body_text + {AI_FIELDS.filter(f => f.startsWith("ai_")).length} AI полей · URL ← ad_archive_id
              </span>
            </div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      {result && (
        <div style={{ fontSize: 13, fontWeight: 700, color: "#6EC8A0", background: "rgba(110,200,160,0.06)", border: "1px solid rgba(110,200,160,0.2)", borderRadius: 8, padding: "10px 14px" }}>
          ✓ Импортировано {result.imported} из {result.total} строк. Claude классифицировал типы (hook/angle/persona…) автоматически.
        </div>
      )}
...
function N8nGuideModal({ onClose }: { onClose: () => void }) {
  const webhookUrl = typeof window !== "undefined"
    ? `${window.location.origin}/api/competitors/import`
    : "/api/competitors/import";

  const examplePayload = JSON.stringify({
    ads: [
      {
        competitor_name: "БухПро",
        ad_text: "Хотите работать бухгалтером в Германии? За 3 месяца с нуля...",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ fontSize: 12, color: "#AAA", lineHeight: 1.7 }}>
        Автоматический сбор рекламы конкурентов через Apify + n8n. Каждый день новые объявления появляются в базе уже классифицированными Claude.
      </div>

      {/* Step 1 */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#E8AA42", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
          Шаг 1 — Apify: Facebook Ads Library Scraper
        </div>
        <div style={{ fontSize: 12, color: "#999", lineHeight: 1.7 }}>
          1. Зарегистрируйтесь на <b style={{ color: "#DDD" }}>apify.com</b><br />
          2. Найдите актор <b style={{ color: "#DDD" }}>Facebook Ads Library Scraper</b><br />
          3. Настройте поиск: keywords = &quot;бухгалтер Германия&quot;, &quot;курсы бухгалтерии&quot;<br />
          4. Возьмите <b style={{ color: "#DDD" }}>API Token</b> и <b style={{ color: "#DDD" }}>Actor ID</b> из настроек<br />
          5. n8n будет запускать этот актор по расписанию
        </div>
      </div>

      {/* Step 2 */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#48B8D0", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
          Шаг 2 — n8n Workflow
        </div>
        <div style={{ fontSize: 12, color: "#999", lineHeight: 1.7 }}>
          Создайте workflow с такими нодами:<br /><br />
          <b style={{ color: "#DDD" }}>① Schedule Trigger</b> — каждый день в 09:00<br />
          <b style={{ color: "#DDD" }}>② HTTP Request</b> — запустить Apify актор (POST)<br />
          <b style={{ color: "#DDD" }}>③ Wait</b> — 60 секунд (пока актор работает)<br />
          <b style={{ color: "#DDD" }}>④ HTTP Request</b> — получить результаты Apify (GET)<br />
          <b style={{ color: "#DDD" }}>⑤ Code</b> — трансформировать данные в формат ниже<br />
          <b style={{ color: "#DDD" }}>⑥ HTTP Request</b> — POST на наш webhook
        </div>
      </div>

      {/* Step 3 */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#6EC8A0", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
          Шаг 3 — Webhook endpoint
        </div>
        <div style={{ fontSize: 12, color: "#666", marginBottom: 6 }}>URL:</div>
        <div style={{ fontFamily: "monospace", fontSize: 11, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "8px 12px", color: "#E8AA42", wordBreak: "break-all" }}>
          {webhookUrl}
        </div>
        <div style={{ fontSize: 12, color: "#666", marginTop: 10, marginBottom: 6 }}>Header:</div>
        <div style={{ fontFamily: "monospace", fontSize: 11, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "8px 12px", color: "#AAA" }}>
          Authorization: Bearer {"{WEBHOOK_SECRET из .env.local}"}
        </div>
        <div style={{ fontSize: 12, color: "#666", marginTop: 10, marginBottom: 6 }}>Body (JSON):</div>
        <pre style={{ fontFamily: "monospace", fontSize: 11, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "12px", color: "#AAA", margin: 0, overflow: "auto" }}>
          {examplePayload}
        </pre>
      </div>
      ...
      {/* Step 4 */}
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#C490D1", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
          Шаг 4 — Code нода в n8n (трансформация)
        </div>
        <pre style={{ fontFamily: "monospace", fontSize: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "12px", color: "#AAA", margin: 0, overflow: "auto" }}>
{`// n8n Code нода — трансформация Apify → наш формат
const items = $input.all();
const ads = items.flatMap(item => {
  const data = item.json;
  // Apify возвращает массив объявлений
  const list = Array.isArray(data) ? data : [data];
  return list.map(ad => ({
    competitor_name: ad.pageName ?? ad.page_name ?? "Unknown",
    ad_text: ad.snapshot?.body?.markup?.__html
      ?? ad.bodyText
      ?? ad.text
      ?? "",
    ad_url: \`https://www.facebook.com/ads/library/?id=\${ad.adArchiveID}\`,
  })).filter(a => a.ad_text.length > 20);
});
return [{ json: { ads } }];`}
        </pre>
      </div>
      ...
      <div style={{ fontSize: 11, color: "#555", lineHeight: 1.6 }}>
        Claude автоматически классифицирует каждое объявление (hook/angle/persona/body/format) и создаёт запись со статусом &quot;Ожидает оценки&quot;. Вы получаете уведомление и заходите одобрить/отклонить.
      </div>
...
export default function CompetitorsPage() {
  ...
  const handleSave = async (data: Omit<Concept, "id" | "createdAt">) => {
    if (modal.editing) {
  ...
  const handleGenerateHypothesis = async () => {
    ...
    setHypothesis("");
    try {
      const h = await generateHypothesis(approved);
      setHypothesis(h);
    } finally {
      setGenerating(false);
    }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Идеи и паттерны из рекламы конкурентов"
        action={
          <div style={{ display: "flex", gap: 8 }}>
            {counts.approved > 0 && (
              <Button onClick={handleGenerateHypothesis} disabled={generating}
                ...
                {generating ? "⏳ Генерирую..." : `✨ Гипотезы из ${counts.approved} идей`}
              </Button>
            )}
            ...
            <Button variant="primary" onClick={() => setModal({ open: true })}>+ Добавить идею</Button>
          </div>
        }
      />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
        {[
          { label: "Ожидают оценки", value: counts.pending, color: "#E8AA42" },
      ...
      {hypothesis && (
        <Card style={{ marginBottom: 20, background: "rgba(232,170,66,0.04)", border: "1px solid rgba(232,170,66,0.2)" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#E8AA42", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>
            ✨ AI Гипотезы на основе одобренных идей
          </div>
          <div style={{ fontSize: 12, color: "#AAA", lineHeight: 1.8, whiteSpace: "pre-wrap" }}>{hypothesis}</div>
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            <Button size="sm" onClick={() => navigator.clipboard.writeText(hypothesis)}>📋 Скопировать</Button>
            <Button size="sm" onClick={() => setHypothesis("")}>✕ Закрыть</Button>
          </div>
        </Card>
      )}
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 20 }}>
        {(["all", "pending", "approved", "rejected"] as const).map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{
            padding: "6px 14px", borderRadius: 8, fontSize: 11, fontWeight: filter === s ? 700 : 400,
            ...
            {s === "all" ? `Все (${concepts.length})` : s === "pending" ? `Ожидают (${counts.pending})` : s === "approved" ? `Одобрены (${counts.approved})` : `Отклонены (${counts.rejected})`}
          </button>
        ))}
        <div style={{ width: 1, background: "rgba(255,255,255,0.06)", margin: "0 4px" }} />
        {(["all", "hook", "angle", "persona", "body", "format"] as const).map(t => (
          <button key={t} onClick={() => setTypeFilter(t)} style={{
            padding: "6px 12px", borderRadius: 8, fontSize: 11, fontWeight: typeFilter === t ? 700 : 400,
            cursor: "pointer", fontFamily: "inherit",
            border: typeFilter === t ? `1px solid ${TYPE_COLOR[t] ?? "#555"}40` : "1px solid rgba(255,255,255,0.06)",
            background: typeFilter === t ? `${TYPE_COLOR[t] ?? "#555"}12` : "rgba(255,255,255,0.02)",
            color: typeFilter === t ? (TYPE_COLOR[t] ?? "#E8AA42") : "#666",
          }}>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map(c => (
            <Card key={c.id} style={{
              border: c.status === "approved"
                ? "1px solid rgba(110,200,160,0.2)"
                : c.status === "rejected"
                ? "1px solid rgba(217,107,107,0.1)"
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                    ...
                    {c.conceptType && (
                      <Badge color={TYPE_COLOR[c.conceptType] ?? "#555"}>
                        {TYPE_LABELS[c.conceptType] ?? c.conceptType}
                      </Badge>
                    )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5, flexShrink: 0 }}>
                  {c.status !== "approved" && (
                    <Button size="sm" variant="primary" onClick={() => quickStatus(c.id, "approved")}>✓ Одобрить</Button>
                  )}
                  {c.status !== "rejected" && (
                    <Button size="sm" onClick={() => quickStatus(c.id, "rejected")}>✕ Отклонить</Button>
                  )}
                  {c.status !== "pending" && (
                    <Button size="sm" onClick={() => quickStatus(c.id, "pending")} style={{ opacity: 0.5 }}>↺</Button>
                  )}
