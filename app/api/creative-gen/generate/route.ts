...
async function generatePromptWithClaude(brief: Record<string, unknown> & { personaName?: string }, isStatic: boolean): Promise<string> {
  ...
  const instruction = isStatic
    ? `Собери HIGH-END промт для nano-banana (Google Gemini image): фотореалистичный ПРЕМИАЛЬНЫЙ статичный креатив, вертикаль 4:5 (1080x1350). Верни ТОЛЬКО финальный промт (без преамбулы, без кавычек). Английский описывает сцену; русский рекламный текст вставляется ДОСЛОВНО в подписанные блоки.
